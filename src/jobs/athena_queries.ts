...
export const athenaQueryToGetMetricsForLastSucessfulJobRun = (timestampInfo: JobTimestampInfo) => {
  const query = `SELECT payload_tour_id, ymd, COUNT(sid) AS views_all, COUNT(distinct aid) AS views_unique FROM 
                   (SELECT aid, payload_tour_id, sid, ymd FROM "ann_btn_clicked" 
...
export const athenaQueryToGetConversionForLastSucessfulJobRun = (timestampInfo: JobTimestampInfo) => {
  const query = `SELECT payload_tour_id,payload_btn_id,ymd, COUNT(payload_ann_id) 
                   ...
                   FROM "ann_btn_clicked" WHERE payload_btn_type!='prev' AND 
                   cast(concat(cast(ymd as varchar), lpad(cast(h as varchar(2)), 2, '0') ) 
                   as bigint)=${parseInt(timestampInfo.lastSuccessfulRunAt)}) 
                   subquery GROUP BY payload_tour_id, payload_btn_id, 
                   ymd ORDER BY ymd; `;
  return query;
};
  
export const athenaQueryToGetAnnTourClicksForLastSucessfulJobRun = (timestampInfo: JobTimestampInfo) => {
  const query = `WITH ranked_data AS ( SELECT payload_ann_id, payload_tour_id, ymd, COUNT(sid) as views_all, 
                   COUNT(distinct aid) as views_unique FROM "ann_btn_clicked" WHERE payload_btn_type != 'prev'
                   AND (cast(concat(cast(ymd as varchar), lpad(cast(h as varchar(2)), 2, '0') ) as bigint)) 
                   = ${parseInt(timestampInfo.lastSuccessfulRunAt)} GROUP BY payload_ann_id, payload_tour_id, ymd) 
                   SELECT DISTINCT payload_tour_id, ymd,payload_ann_id, views_all, views_unique, ARRAY[
                   approx_percentile(views_all, 0.01) OVER (PARTITION BY payload_tour_id),
...
export const athenaQueryToGetMetricsFromLastSuccessToCurrentTimestamp = (timestampInfo: JobTimestampInfo) =>{
  const query = `SELECT payload_tour_id, ymd, COUNT(sid) AS views_all, COUNT(distinct aid) AS views_unique FROM 
                   ...
                   >=${parseInt(timestampInfo.currentRunAt)}) subquery GROUP BY payload_tour_id, ymd`;
  return query;
};
  
export const athenaQueryToGetConversionFromLastSuccessToCurrentTimestamp = 
  (timestampInfo: JobTimestampInfo) =>{
    const query = `SELECT payload_tour_id,payload_btn_id,ymd, COUNT(payload_ann_id) 
                   as clicks FROM (select payload_tour_id, payload_ann_id, payload_btn_id,ymd 
                   FROM "ann_btn_clicked" WHERE payload_btn_type!='prev' AND 
                   cast(concat(cast(ymd as varchar), lpad(cast(h as varchar(2)), 2, '0') ) 
                   as bigint)>=${parseInt(timestampInfo.currentRunAt)}) 
                   subquery GROUP BY payload_tour_id, payload_btn_id, 
                   ymd ORDER BY ymd; `;
    return query;
  };
  
export const athenaQueryToGetAnnClicksFromLastSuccessToCurrentTimestamp = 
  (timestampInfo: JobTimestampInfo) =>{
    const query = `WITH ranked_data AS ( SELECT payload_ann_id, payload_tour_id, ymd, COUNT(sid) as views_all, 
                   COUNT(distinct aid) as views_unique FROM "ann_btn_clicked" WHERE payload_btn_type != 'prev'
                   AND (cast(concat(cast(ymd as varchar), lpad(cast(h as varchar(2)), 2, '0') ) as bigint)) 
                   >= ${parseInt(timestampInfo.currentRunAt)} GROUP BY payload_ann_id, payload_tour_id, ymd) 
                   SELECT DISTINCT payload_tour_id, ymd,payload_ann_id, views_all, views_unique, ARRAY[
                   approx_percentile(views_all, 0.01) OVER (PARTITION BY payload_tour_id),
                   approx_percentile(views_all, 0.05) OVER (PARTITION BY payload_tour_id),
                   approx_percentile(views_all, 0.10) OVER (PARTITION BY payload_tour_id),
