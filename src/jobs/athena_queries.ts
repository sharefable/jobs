export const athenaQueryToGetMetricsData = (successJobcurrentRunAt: string, currentRanFor: string) => {
  const query = `SELECT payload_tour_id, ymd, COUNT(sid) AS views_all, COUNT(distinct aid) AS views_unique FROM 
                   ...
                   lpad(cast(h as varchar(2)), 2, '0') ) as bigint)
                   <= ${currentRanFor}) subquery GROUP BY payload_tour_id, ymd`;
  return query;
};
  
export const athenaQueryToGetConversionData = (successJobcurrentRunAt: string, currentRanFor: string) => {
...
export const athenaQueryToGetAnnTourClicksData= (successJobcurrentRunAt: string, currentRanFor: string) => {
  const query = `WITH ranked_data AS ( SELECT payload_ann_id, payload_tour_id, ymd, COUNT(sid) as views_all, 
                   ...
                   >= ${successJobcurrentRunAt} AND cast(concat(cast(ymd as varchar), 
                   lpad(cast(h as varchar(2)), 2, '0') ) as bigint)
                   <= ${currentRanFor} GROUP BY payload_ann_id, payload_tour_id, ymd) 
